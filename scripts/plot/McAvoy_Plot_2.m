...
function McAvoy_Plot_2(y_history,u_history,setpoint,my_label,MV_label,sampling)
warning off
k_y=(size(y_history,1)-1)*sampling; %ending minute
k_u=(size(u_history,1)-1)*sampling; %ending minute
y_lowlimit=[0.072*100 140 109.9 0.000008*100000 0.08*100 0.4*1000 0.7*1000 0.6*1000];
y_highlimit=[0.078*100 170 110.05 0.000017*100000 0.105*100 0.6*1000 0.92*1000 0.9*1000];
figure(1);
subplot(4,2,1);
...
subplot(4,2,4);
plot(0:sampling:k_y,y_history(:,27)*1e5);
ylabel(my_label(27,:));
xlabel('minute')
...
%title('Reactor Outlet Temperature Increases 6 degC')
figure(2)
close(2);
warning on

...
time=0;
% set initialization flag 1/// when set to 0 function freeze
is_initial=1;
...
yLO_DecanterTemp=40;
yHI_DecanterTemp=50;
Nded_DecanterTemp=1+transmitter_deadtime(22)*transmitter_sampling_frequency(22);
Ntau_DecanterTemp=1;
...
%-----------------------------------------------------------------------------
%controller initialization
%MV23: Organic Level is controlled by Organic Product Flow at 0.5
SP_Organic=0.5;
K_Organic=1;
Ti_Organic=100; 
act_Organic=-1;
...
%-----------------------------------------------------------------------------
%controller initialization
%MV24: Aqueous Level is controlled by Aqueous Product Flow at 0.5
SP_Aqueous=0.5;
K_Aqueous=1;
Ti_Aqueous=100; 
act_Aqueous=-1;
...
%-----------------------------------------------------------------------------
%MV25: Column Bottom Exit is used to control the Column Bottom Level
SP_ColButtom=0.5;
K_ColButtom=1;
Ti_ColButtom=100; 
act_ColButtom=-1;
...
x_history=zeros(minute*model_sampling_frequency,246);
y_history=zeros(minute*model_sampling_frequency,27);
u_history=zeros(minute*model_sampling_frequency,26);
dx_history=zeros(minute*model_sampling_frequency,246);
...
%        info=['minute: ' num2str(time) '    max_dxdt_error: ' num2str(error) '    location: ' num2str(indx)];
 %       disp(info);
    ...
    if rem(k,storage_sampling_frequency/model_sampling_frequency)==0
        ...
        u_history(outInd,:)=MVs';
        dx_history(outInd,:)=dstatedt';
